...
describe("Alerts System", () => {
  ...
  beforeEach(() => {
    ...
    const user = {
      name: "keerthi",
      income: 20000,
      categories: [{ title: "Food", budgetAmount: 8000, spentTillNow: 9000 }],
      savings: [{ title: "Emergency", target: 10000, current: 8000 }],
      otherIncome: 5000,
      totalIncome: 30000,
      otherSavings: 5000,
    };
  ...
  it("should alert when savings goal is close to being reached", () => {
    ...
    if (user) {
      ...
      expect(alerts).toContain(
        "You are close to reaching your savings goal for Emergency!"
      );
    }
  });
